...
resource "aws_security_group" "test-project-bastionhosts-sg" {
...
resource "aws_instance" "test-project-bastionhost-1a" {
  ami = "ami-0e3f630ea5003ecf3"
  instance_type = "t2.micro"
  key_name = "Migration"
  associate_public_ip_address = true
  vpc_security_group_ids = [aws_security_group.test-project-bastionhosts-sg.id]
  subnet_id = aws_subnet.test-project-pubsubnet1A.id
  tags = {
    "Name" = "TEST-PROJECT-BASTION-HOST-1A"
  }
}

# Create Bastion Host in eu-west-1b public subnet

resource "aws_instance" "test-project-bastionhost-1b" {
  ami = "ami-0e3f630ea5003ecf3"
  instance_type = "t2.micro"
  key_name = "Migration"
  associate_public_ip_address = true
  ...
  tags = {
    "Name" = "TEST-PROJECT-BASTION-HOST-1B"
  }
}